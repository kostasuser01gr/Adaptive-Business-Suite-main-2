...
export async function getWorkspaceContext(userId: string, query: string): Promise<string> {
  ...
  if (q.includes('booking') || q.includes('rental') || q.includes('status')) {
    const activeBookings = bookings.filter(b => b.status === 'active').slice(0, 5);
  ...
  if (todoTasks.length > 0 && (q.includes('task') || q.includes('todo') || q.includes('do'))) {
    contextBlock += "\n**Priority Tasks:**\n";
    todoTasks.forEach(t => {
