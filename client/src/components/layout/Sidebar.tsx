...
import {
  CarFront, LayoutDashboard, Calendar, FileText,
  Settings, Bot, Briefcase, User, Wallet, LogOut, Menu, X,
  Wrench, CheckSquare, Users, ChevronDown, Sparkles, Shield
} from 'lucide-react';
...
export default function Sidebar() {
    ...
    <>
            ...
            <Link key={link.path} href={link.path}>
              <div
                ...
                className={`flex items-center px-3 py-2 rounded-lg transition-all duration-150 cursor-pointer text-[13px] ${
                  isActive ? 'bg-primary/10 text-primary font-medium' : 'text-muted-foreground hover:bg-white/[0.04] hover:text-foreground'
                }`}
      ...
      <div className="p-3 border-t border-white/5 space-y-2 shrink-0">
        ...
        <div className="flex items-center justify-between px-1 pt-1">
          <span className="text-[11px] text-muted-foreground truncate max-w-[140px]">{user?.displayName || user?.username}</span>
          <Button variant="ghost" size="icon" className="h-7 w-7 hover:text-destructive" onClick={() => logout()} data-testid="button-logout">
  ...
  return (
    <>
      <button className="lg:hidden fixed top-3 left-3 z-50 p-2 glass rounded-lg" onClick={() => setMobileOpen(true)} data-testid="button-mobile-menu">
        <Menu className="h-4 w-4" />
      </button>
      {mobileOpen && <div className="lg:hidden fixed inset-0 bg-black/50 z-40 backdrop-blur-sm" onClick={() => setMobileOpen(false)} />}
      <aside className={`w-56 bg-background/80 backdrop-blur-xl border-r border-white/5 flex flex-col fixed lg:static inset-y-0 left-0 z-50 transition-transform duration-200 ${mobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        {sidebarContent}
      </aside>
    </>
  );
